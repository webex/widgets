...
test.describe('User State Widget - Single Login, Multiple Tests', () => {
  // Helper to wait for a console message containing expected text
 /* async function waitForConsoleMessage(expected: string, messages: string[], timeout = 5000) {
    const start = Date.now();
    while (Date.now() - start < timeout) {
      if (messages.some(msg => msg.includes(expected))) return true;
...
*/
  ...
  test('should change state to Available and verify theme and timer reset', async () => {
    ...
    // Parse timer strings in "mm:ss" format to seconds for comparison
  ...
  test('should verify state persistence after page reload', async () => {
    ...
    await changestate(page, 'Available');
    await page.waitForTimeout(2000);
    verifyCurrentState(page, 'Available');
    await page.reload();
  ...
  test('should validate callback sequence', async () => {
    ...
    await changestate(page, 'Available');
    await page.waitForTimeout(2000);
    // Wait for callback for 'Available' before checking sequence
