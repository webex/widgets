...
test.describe('Task List Tests for different types of Task', () => {
  ...
  test('Verify Task List for incoming Call', async () => {
    ...
    await expect(page.getByTestId(`${interactionId}-state`)).toHaveText('Connected');
    await expect(page.getByTestId(`${interactionId}-handle-time`)).toBeVisible();
  ...
  test('Task List Test with Multiple Taks', async () => {
    ...
    await Promise.all([createCallTask(callerpage), createChatTask(chatPage), createEmailTask()]);
    ...
    for (let i = 0; i < 3; i++) {
      const taskListItem = page.getByTestId('task-list').getByRole('listitem').nth(i);
