...
export const desktopLogin = async (page: Page): Promise<void> => {
  ...
  await page.getByTestId('teams-dropdown-select').locator('#select-base-triggerid div').click();
...
export const extensionLogin = async (page: Page): Promise<void> => {
  ...
  await page.getByRole('textbox', { name: 'Extension' }).fill(extensionNumber);
  await page.getByTestId('teams-dropdown-select').locator('#select-base-triggerid div').click();
  await page.waitForTimeout(200);
  await page.locator('[data-testid^="teams-dropdown-"]').nth(1).locator('span, div').first().click();
  await page.getByTestId('login-button').click();
...
export const stationLogout = async (page: Page): Promise<void> => {
  ...
  await page.getByRole('button', { name: 'Station Logout' }).click();
