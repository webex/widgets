...
| `cc-components.consult-transfer-lists` | SDK callback props | `getQueuesFetcher`, `getQueues`, and `getEntryPoints` use `FetchPaginatedList<ConsultTransferDestination>` | Render SDK-projected queue and entry-point rows without assuming full CMS record fields | additive alignment with the SDK/store projection; changing required destination fields is semver-sensitive | `src/components/task/task.types.ts` | [`CONTRACTS.md`](../../../../ai-docs/CONTRACTS.md) |
...
- `@webex/cc-store` (workspace:\*) — type-only import surface here (`ITask`, `ILogger`, `IContactCenter`, `ConsultTransferDestination`, `IdleCode`, `IWrapupCode`, `BuddyDetails`, etc.) and constants such as `ERROR_TRIGGERING_IDLE_CODES`, `LoginOptions`, `DESKTOP`. Components consume types/constants, not the store singleton.
...
| `CC-COMPONENTS-R-016` | Consult/transfer paginated hooks must keep SDK response rows directly, append later pages without sorting/filtering/reprojection, and render queue/entry-point/dial-number arrays in received order. | The component must preserve backend-selected order and exact projected rows instead of introducing a second list policy. | `src/components/task/task.types.ts`, `src/components/task/CallControl/CallControlCustom/consult-transfer-popover-hooks.ts` | `tests/components/task/CallControl/CallControlCustom/consult-transfer-popover.tsx`, `tests/components/task/CallControl/CallControlCustom/consult-transfer-popover.snapshot.tsx` | Buddy-agent text search remains a presentational filter over the already ordered agent response. | PRESENT |
...
| `CC-COMPONENTS-R-016` | `tests/components/task/CallControl/CallControlCustom/consult-transfer-popover.tsx`, `tests/components/task/CallControl/CallControlCustom/consult-transfer-popover.snapshot.tsx` | None |
