...
const IncomingTaskComponent: React.FunctionComponent<IncomingTaskComponentProps> = (props) => {
  const {incomingTask, isBrowser, accept, reject, logger, isEnableDeclineButton} = props;
  if (!incomingTask) {
    return <></>; // hidden component
  }

  // Extract all task data using the utility function
  const taskData = extractIncomingTaskData(incomingTask, isBrowser, logger, isEnableDeclineButton);
