...
| `STORE-R-022` | `tests/storeEventsWrapper.ts`, `tests/util.ts` | Add a widget integration assertion for direct Task destination controls if the store ever begins adapting task UI controls. |
| `STORE-R-023` | `tests/storeEventsWrapper.ts` (`handleWxAppMuteStateUpdated`) | none |
| `STORE-R-024` | `tests/storeEventsWrapper.ts` (`seed isMuted on setCurrentTask`, dedupe tests) | none |
