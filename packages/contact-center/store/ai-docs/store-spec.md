...
| `STORE-R-027` | **`setCurrentTask`** resets **`isMuted`** to `false` synchronously when the promoted task **changes** (`!isSameTask`), then calls **`seedWxAppMuteFromTask`**: resolves canonical SDK task from `taskManager.getAllTasks()`, awaits **`syncWxAppMuteFromCallDetails()`**, and applies **`getWxAppMuted()`** to `store.isMuted` only while that task remains current (refresh/hydrate backfill; skips re-seed on uiControls-driven list refresh). The synchronous reset prevents the prior task's mute indicator from leaking onto the newly selected task before async seed completes. | Page refresh must restore mute icon from telephony GET even if Mercury event was missed; task switches must not show stale mute state; avoid duplicate GETs on answer | `src/storeEventsWrapper.ts` (`seedWxAppMuteFromTask`, `setCurrentTask`) | `tests/storeEventsWrapper.ts` (`seed isMuted on setCurrentTask`, task-switch stale mute reset, dedupe tests) | SDK Voice must implement sync + getter + in-flight dedupe | PRESENT |
...
| `STORE-R-027` | `tests/storeEventsWrapper.ts` (`seed isMuted on setCurrentTask`, task-switch stale mute reset, dedupe tests) | none |
