...
- `startOutdial(destination, origin?)` - Start outdial function (matches `OutdialCallProps.startOutdial`)
- `getOutdialANIEntries()` - Fetch ANI entries async function
- `isTelephonyTaskActive` - Boolean flag indicating if there is any active telephony task (used to disable outdial when true)
...
sequenceDiagram
    ...
    U->>W: Render widget
    W->>H: Initialize hook
    H-->>W: Return {startOutdial, getOutdialANIEntries, isTelephonyTaskActive}
    W->>C: Pass props
    C->>C: useEffect - mount
