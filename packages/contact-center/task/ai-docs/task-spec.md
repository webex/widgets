...
flowchart LR
  ...
  Hook -->|setTaskCallback EVENT, fn, task| Store
...
| `toggleMute` SDK failure                                                                                               | `onToggleMute` fires with the _previous_ `isMuted`; store not updated              | UI stays consistent with actual mute state                    |
...
- DON'T add new task-event subscriptions without matching the exact event name in both `setTaskCallback` and the cleanup `removeTaskCallback`. Always pass the task object (not an ID string) and the same callback reference to both.
...
| `TASK-R-007` recording toggle                  | `tests/helper.ts` (pause/resume + failure cases)                                                                          | No test asserts the PAUSED/RESUMED vs CONTACT\_\* cleanup mismatch |
