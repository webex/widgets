...
jest.mock('../../src/CallControl/CallControlCustomComponents/call-control-list-item.presentational', () => {
  const MockListItem = (props: any) => (
    <div data-testid="CallControlListItemPresentational" onClick={props.onButtonPress}>
      {props.title}
    </div>
  );
  MockListItem.displayName = 'CallControlListItemPresentational';
  return MockListItem;
});
...
describe('CallControlPopoverPresentational', () => {
  ...
  it('renders heading and tab correctly', () => {
    render(<CallControlPopoverPresentational {...baseProps} />);
  ...
  it('renders no agents text when buddyAgents is empty', () => {
    render(<CallControlPopoverPresentational {...baseProps} buddyAgents={[]} />);
